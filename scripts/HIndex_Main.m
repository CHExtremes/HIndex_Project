...
for i = 1:length(tableStationNames) %for each station in station names 
    ...
    Results.HIndexPValue(i) = round(table2array(mdl.Coefficients(2,4)),3); %calls the pValue from the fitlm function and inputs it into a new table
...
for i = 1:stationLength %for each station in station names
    ...
    Results.CIndexPValue(i) = round(table2array(mdl.Coefficients(2,4)),3); %calls the pValue from the fitlm function and inputs it into a new table
...
for i = 1:stationLength %for each station in station names
    ...
    %sown plot
    tempNames = split(tableStationNames(i), '_');
    A = array2table(WIndex(CIndex(:,1+i) ~= 0,[ 1 1+i])); %for each station, create a new array with the non-zero values and their corresponding years
    A.Properties.VariableNames{'Var1'} = 'YEAR'; 
