...
for i = 1:length(tableStationNames) %for each station in station names 
    ...
    Results.minPOR(i) = min(A.HIndex);
    Results.maxPOR(i) = max(A.HIndex);
    Results.meanPOR(i) = round(mean(A.HIndex),1); %calculates the average H index for the station and adds that to a new table
    Results.medianPOR(i) = median(A.HIndex);
    ...
    Results.minSP(i) = min(B.HIndex);
    Results.maxSP(i) = max(B.HIndex);
    Results.meanSP(i) = round(mean(B.HIndex),1);
    Results.medianSP(i) = median(B.HIndex);    
